...
async function processLineByLine() {
	...
	while (que.length) {
		const [index, steps, bitmask] = que.pop();
		...
		for (const [toIndex, value] of es) {
			if ((bitmask & (1 << toIndex)) === 0) {
				const new_bitmask = bitmask | (1 << toIndex);
				const new_steps = steps + value;
				que.push([toIndex, new_steps, new_bitmask]);
				console.log(`visiting from ${index} to ${toIndex}, bitmask ${new_bitmask.toString(2)}`);
			}
		}
	}
